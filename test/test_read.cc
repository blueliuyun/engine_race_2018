...
static uint64_t nWriteCnt = 0;
static uint64_t nReadCnt = 0;
static uint64_t nReadLostCnt = 0;
...
public:
    ...
    RandNum_generator(int start, int end)
        ...
        , e(std::hash<std::thread::id>()(std::this_thread::get_id())
            + std::chrono::duration_cast<std::chrono::milliseconds>(std::
...
unsigned numRead)
{
    ...
    for (unsigned i = 0; i < numRead; ++i) {
        ...
        std::string key(std::to_string(nReadCnt));
		
        std::string val;
		//std::cout << "Before [ engine->Read(key, &val) ] " << std::endl;
        engine->Read(key, &val);		
		std::cout << "Key= " << key << "	Val= " << val << std::endl;
		if(key != val){
			nReadLostCnt++;
...
{
public:
    MyVisitor(const threadsafe_vector<std::string>& keys, unsigned start, 
unsigned& cnt)
    ...
    void Visit(const PolarString& key, const PolarString& value)
    {
        ...
        if (key != key_from_value(value.ToString())) {
            std::cout << "Sequential Read error: key and value not match" << 
std::endl;
            exit(-1);
        }
        if (key != mKeys[mStart + mCnt]) {
            std::cout << "Sequential Read error: not an expected key" << std::
endl;
            exit(-1);
        }
        mCnt += 1;
...
/*    
    ...
    std::cout << "Random read takes: " << std::chrono::duration<double, std::milli>(rreadEnd - rreadStart).count()
              << " milliseconds" << std::endl;    
...
*/
    ...
    std::cout << "Sequential read takes: " << std::chrono::duration<double, std::milli>(sreadEnd - sreadStart).count()
              << " milliseconds" << std::endl;
